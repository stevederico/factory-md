...
- **Out:** `plan.md`.
- **Gate (human):** you read/edit/approve `plan.md`. This is the cheapest place to correct course — do it here, not after code exists.

### 3. Build  →  `factory run plan.md`  (part 1)
- **In:** approved `plan.md` + `factory.md` as context.
...
- **Decision:**
  ...
  - `exit = 0` → the diff is provably meets the plan. Continue to Ship.
- **Loop guard:** cap at N attempts (e.g. 5). If still failing, stop and hand the failures to a human.

### 5. Ship  →  (end of `factory run`)
- **In:** a green diff.
- **Action:** commit, push branch, open PR. PR body = the plan + the grader's green report.
...
Two gates only:
1. **Approve plan** (stage 2) — steer before any code exists.
...
**Next:** wire stages 2–5 into the `factory` CLI so `factory plan` → `factory run` completes the loop around the grader that already works.
